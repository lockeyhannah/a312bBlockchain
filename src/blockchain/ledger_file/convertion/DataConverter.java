...
public class DataConverter extends Converter<Data> {
    ...
    private final ArrayList<Converter> converters = new ArrayList<>();
    ...
    public byte[] bytesFromInstance(Data data) {
        ...
        for (DataPoint dp : dataPoints) {
            ...
            Converter converter = getConverter(dp);

            if (converter != null) {
                // Add DataPoint Id to byte array
                byte[] converterTypeBytes = ByteUtils.toByteArray(converter.getOBJECT_TYPE_UID());
                converterTypeBytes = ByteUtils.extendByteArray(converterTypeBytes, OBJECT_TYPE_UID_BYTE_LEN);
                bytes.add(converterTypeBytes);

                // Convert data point to bytes
                byte[] dpBytes = converter.bytesFromInstance(dp);

                // Add length of data point to byte array
                short dpByteLen = (short) dpBytes.length;
                bytes.add(ByteUtils.extendByteArray(ByteUtils.toByteArray(dpByteLen), DATA_POINT_SIZE_BYTE_LEN));
